define([
    ...
    for (var selectorIdx = 0; selectorIdx < selectors.length; selectorIdx++) {
        require(["../../../query!" + selectors[selectorIdx]], function (query) {
            ...
            registerSuite({
                name: "dojo/query - selector: " + selectors[selectorIdx],
                setup: function () {
                    if (!container) {
                        container = domConstruct.toDom(queryRawHTML);
                ...
                "css2": {
                    ...
                    "descendant selectors": function () {
                        assert.equal(query('> *', 'container').length, 3);
                        assert.equal(query('> [qux]', 'container').length, 2);
                        assert.equal(query('> [qux]', 'container')[0].id, "child1");
                        assert.equal(query('> [qux]', 'container')[1].id, "child3");
                        assert.equal(query('> *', 'container').length, 3);
                        assert.equal(query('>*', 'container').length, 3);
                    ...
                    "bug 9071": function () {
                        // bug 9071
                        assert.equal((query('a', 't4')).length, 2);
                        assert.equal((query('p a', 't4')).length, 2);
                        assert.equal((query('div p', 't4')).length, 2);
                        assert.equal((query('div p a', 't4')).length, 2);
                        assert.equal((query('.subA', 't4')).length, 2);
                        assert.equal((query('.subP .subA', 't4')).length, 2);
                        assert.equal((query('.subDiv .subP', 't4')).length, 2);
                        assert.equal((query('.subDiv .subP .subA', 't4')).length, 2);
                    },
                    ...
                    "escaping special characters with quotes": function () {
                        ...
                        // bug 10651
                        assert.equal(query('option[value="a+b"]', "attrSpecialChars").length, 1);
                        assert.equal(query('option[value="a~b"]', "attrSpecialChars").length, 1);
                        assert.equal(query('option[value="a^b"]', "attrSpecialChars").length, 1);
                        assert.equal(query('option[value="a,b"]', "attrSpecialChars").length, 1);
                    },
                    "selector with substring that contains equals sign - bug 7479": function () {
                        assert.equal(1, query("a[href*='foo=bar']", 'attrSpecialChars').length);
                    },
                    "selector with substring that contains brackets - bug 9193, 11189, 13084": function () {
                        assert.equal(query('input[name="data[foo][bar]"]', "attrSpecialChars").length, 1);
                        assert.equal(query('input[name="foo[0].bar"]', "attrSpecialChars").length, 1);
                        assert.equal(query('input[name="test[0]"]', "attrSpecialChars").length, 1);
                    },
                    ...
                    "document_fragment": function () {
                        ...
                        assert.equal(1, query("#b", detachedDom).length);
                        assert.equal(1, query("#b", detachedDom.firstChild).length);
                        assert.equal(1, query("#b", documentFragment).length);
                        assert.equal(1, query("#b", documentFragment.childNodes[2]).length);

                        var detachedDom2 = domConstruct.toDom("<i><u><a></a><b></b></u></i>");
                        var documentFragment2 = domConstruct.toDom("<i></i>    <u><a></a><b></b></u>");

                        assert.equal(0, query("#b", detachedDom2).length);
                        assert.equal(0, query("#b", documentFragment2).length);
                    }
                },
                "css2.1": function () {
                    ...
                    if (/css3|acme/.test(selectors[selectorIdx])) {
                        return {
                            ...
                            "nth-child tests": function () {
                                ...
                                assert.equal(query(':nth-child(2)')[0], query('title')[0]);
                            },
                            ":checked pseudo-selector": function () {
                                assert.equal(query('#t2 > :checked').length, 2);
                                assert.equal(query('#t2 > input[type=checkbox]:checked')[0], dom.byId('checkbox2'));
                                assert.equal(query('#t2 > input[type=radio]:checked')[0], dom.byId('radio2'));
