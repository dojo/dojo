...
function runScrollingTest(resultNode){
	...
	clientWidth.scrollIntoView(true);
	setTimeout(function(){
		var cw = dojo.hitch(dojo, "withGlobal")(window, "position", dojo, [clientWidth, false]);
		if(cw.x < 0){
			scrollBy(cw.x, 0); // webkit bug where the whole element is not fully scrolled into view
		}
...
function genScrollingTestNodes(hScroll, vScroll, large){
	document.write(
		'<DIV id="clientWidth" style="background-color:yellow;">&nbsp;</DIV>' +
...
function genScrollingTestBody(){
	...
	document.write('<BODY style="margin:0;padding:0;border:0;background-color:white;overflow-x:' + (options.horz ? (isQuirks ? scroll : '') : 'hidden') + ';overflow-y:' + (isQuirks ? (options.vert ? scroll : 'hidden') : '') + ';">');
