define([
    ...
    return declare("NumericShaperUtility", null, {
		// summary:
		//		This class handles numeric shaping.  A shaper can either be contextual or not.
		//		A non-contextual shaper will always translate ASCII digits in its input into the target Unicode range.
		//		A contextual shaper will change the target Unicode range depending on the characters it has previously processed. 
		...
		//		The mask of languages to shape for
			...
			// An ASCII digit zero is left unchanged when shaping to this range.
			TAMIL: 256,
			...
			// This is hard-coded because the values will not change.
			// In the two places where a language does not have a zero digit, 
			// the character immediately preceeding the one digit is used instead.
			...
			// If the shaper is not contextual, the high bit of this field will be set.
			mask: 0,
		...
		// summary:
		//		Returns the representation of the Unicode block containing the
		//		given character, or if the character is not a member of a defined block.
		// c: char
		...
		//		The result is taken by "or"ing together the constants representing the various languages
		...
		shape: function(/*String*/ text, /*int*/ start, /*int*/ count) {
		// summary:
		...
		// 		Note that as there is no separate ARABIC block in Character,
		// 		this case must be specially handled by the caller; EASTERN_ARABIC is preferred
		...
		//		the language constant, or zero if not recognized
			...
			if(b == this.blocksEnum.ARABIC)
				return this.EASTERN_ARABIC;
			if(b == this.blocksEnum.BENGALI)
				return this.BENGALI;
			if(b == this.blocksEnum.DEVANAGARI)
				return this.DEVANAGARI;
			if(b == this.blocksEnum.ETHIOPIC)
				return this.ETHIOPIC;
			if(b == this.blocksEnum.BASIC_LATIN
					|| b == this.blocksEnum.LATIN_1_SUPPLEMENT
					|| b == this.blocksEnum.LATIN_EXTENDED_A
					|| b == this.blocksEnum.LATIN_EXTENDED_ADDITIONAL
					|| b == this.blocksEnum.LATIN_EXTENDED_B)
				return this.EUROPEAN;
			if(b == this.blocksEnum.GUJARATI)
				return this.GUJARATI;
			if(b == this.blocksEnum.GURMUKHI)
				return this.GURMUKHI;
			if(b == this.blocksEnum.KANNADA)
				return this.KANNADA;
			if(b == this.blocksEnum.KHMER)
				return this.KHMER;
			if(b == this.blocksEnum.LAO)
				return this.LAO;
			if(b == this.blocksEnum.MALAYALAM)
				return this.MALAYALAM;
			if(b == this.blocksEnum.MONGOLIAN)
				return this.MONGOLIAN;
			if(b == this.blocksEnum.MYANMAR)
				return this.MYANMAR;
			if(b == this.blocksEnum.ORIYA)
				return this.ORIYA;
			if(b == this.blocksEnum.TAMIL)
				return this.TAMIL;
			if(b == this.blocksEnum.TELUGU)
				return this.TELUGU;
			if(b == this.blocksEnum.THAI)
				return this.THAI;
			if(b == this.blocksEnum.TIBETAN)
				return this.TIBETAN;
			return 0;
		},
		...
		// summary:
		//		This method returns the number of zero bits preceding the highest-order ("leftmost") one-bit 
		//		in the two's complement binary representation of the specified int value,
		//		or 32 if the value is equal to zero.
		// i: int
		//      The integer will be passed 
		// return:
		//		the number of zero bits preceding the highest-order ("leftmost") one-bit in the two's complement binary representation
		...
		shapeWith: function(/*String*/ shaperValue, /*String*/ text){
		// summary:
		//		Shape the given text depends on the given shaper value
		// shaperValue: String
		//      the value of the shaper (National , Contextual, Nominal)
		// text: String
		//      the text to be shaped
		// return:
		//		the shaped string
		...
		shapeWithContext: function(/*string*/text, /*int*/ start, /*int*/ count, /*int*/ context) {
		// summary:
		//		Shape the given text, using the indicated initial context. 
		//		If this shaper is not a contextual shaper, then the given context will be ignored
		// text: string
		//      Text to be shaped
		//	start: int
		//		The index of the first character of the text to shape
		// count: int
                ...
                if(c != ' '){
                	if(c >= '0' && c <= '9'){
                		if(currentContext >= 0){	
                			...
                			if(c == '0'
                				&& ((1 << currentContext) == this.TAMIL || (1 << currentContext) == this.ETHIOPIC)){
		...
		shapeOnCtrl: function(/*String*/ shaperValue, /*String*/ text, /*Boolean*/ isRight){
		...
		// summary:
		//		Return a default contextual shaper which can shape to any of the indicated languages.
		//		The default initial vacontext for this shaper is EUROPEAN.
		// ranges: int
		//		the ranges to shape
		...
		getContextualShaper: function(/*int*/ ranges, /*int*/ defaultContext) {
		// summary:
		//		Return a contextual shaper which can shape to any of the indicated languages.
		//		The default initial context for this shaper is given as an argument.
		// ranges: int
		//		The ranges to shape
		// defaultContext: int
		...
		// summary:
		//		Return a non-contextual shaper for the passed single range (language)
